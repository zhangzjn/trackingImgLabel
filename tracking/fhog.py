...
def func1(dx, dy, boundary_x, boundary_y, height, width, numChannels):
    ...
    for j in range(1, height - 1):
        for i in range(1, width - 1):
            ...
            for kk in range(1, NUM_SECTOR):  # changed
...
def func4(mappmap, p, sizeX, sizeY, pp, yp, xp, nx, ny):
    ...
    for i in range(sizeY):
        for j in range(sizeX):
            pos1 = (i * sizeX + j) * p  # 12*NUM_SECTOR
            pos2 = (i * sizeX + j) * pp  # pp = NUM_SECTOR * 3 + 4, yp = 4, xp = NUM_SECTOR
            # nx = 1.0 / np.sqrt(xp * 2), ny = 1.0 / np.sqrt(yp)
...
def getFeatureMaps(image, k, mapp):
    ...
    arg_vector = np.arange(NUM_SECTOR + 1).astype(np.float32) * np.pi / NUM_SECTOR  # 0 pi*1/9 pi*2/9 ... pi*9/9
	...
	'''
    # acquire value and direction of each point
    ### 200x speedup
    r, alfa = func1(dx, dy, boundary_x, boundary_y, height, width, numChannels)  # with @jit
    ### ~0.001s

    nearest = np.ones((k), np.int)
    nearest[0:k // 2] = -1  # [-1, -1, 0, 0]

    w = np.zeros((k, 2), np.float32)
    a_x = np.concatenate((k / 2 - np.arange(k / 2) - 0.5, np.arange(k / 2, k) - k / 2 + 0.5)).astype(
        np.float32)  # [1.5, 0.5, 0.5, 1.5]
    b_x = np.concatenate((k / 2 + np.arange(k / 2) + 0.5, -np.arange(k / 2, k) + k / 2 - 0.5 + k)).astype(
        np.float32)  # [2.5, 3.5, 3.5, 2.5]
    w[:, 0] = 1.0 / a_x * ((a_x * b_x) / (a_x + b_x))  # [0.625, 0.875, 0.875, 0.625]
    w[:, 1] = 1.0 / b_x * ((a_x * b_x) / (a_x + b_x))  # [0.375, 0.125, 0.125, 0.375]
